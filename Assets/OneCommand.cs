...
public class OneCommand : MonoBehaviour
{
    ...
    private void FixedUpdate() { if (CompareTag("GameManager")) FixedUpdate_GM(); }
    ...
    IEnumerator BlockMoveDown(Transform TR)
    {
        ...
        if (targetPos.y < -50)
        {
            if (TR.CompareTag("Block"))
            {
                ...
                Camera.main.GetComponent<Animator>().SetTrigger("shock");
                S_GameOver.Play();
            }
            else
            {
    ...
    void Update_GM()
    {
        ...
        if (isMouse)
        {
            ...
            BallPreview.transform.position = Physics2D.CircleCast(new Vector2(Mathf.Clamp(veryFirstPos.x, -54, 54), groundY), 1.7f, gap, 10000, 1 << LayerMask.NameToLayer("Wall") | 1 << LayerMask.NameToLayer("Block")).centroid; // 1.8
